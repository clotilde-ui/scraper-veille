...
interface ScraperNewJobModalProps {
  ...
  editDefaults?: EditJobDefaults;
}

type Step = 1 | 2 | 3;

export function ScraperNewJobModal({ isOpen, onClose, onSubmit, editDefaults }: ScraperNewJobModalProps) {
  const [step, setStep] = useState<Step>(1);
  const [name, setName] = useState(editDefaults?.name ?? '');
  const [urlsText, setUrlsText] = useState(editDefaults?.urls.join('\n') ?? '');
  const [csvFile, setCsvFile] = useState<File | null>(null);
  const [scrapeTypes, setScrapeTypes] = useState<ScrapeType[]>(editDefaults?.scrapeTypes ?? ['pdfs']);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="bg-white dark:bg-slate-800 border-none shadow-lg max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col p-0 gap-0 sm:rounded-2xl [&>button:last-child]:hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 dark:border-slate-700">
          <div>
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">{isEdit ? 'Modifier et relancer' : 'Nouveau scraping'}</h2>
          ...
          {step === 2 && (
            <>
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder={hasPdfsOnly ? 'Ex: tarif, rapport, bilan' : 'Ex: travaux, contact, travaux AND (mairie OR commune)'}
                      className="w-full px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                    {!hasPdfsOnly && (
                      <p className="text-xs text-slate-400 mt-1">
                        Supporte les opérateurs booléens : <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded">AND</code> <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded">OR</code> <code className="bg-slate-100 dark:bg-slate-700 px-1 rounded">NOT</code> et les parenthèses. Ex : <em>travaux AND (mairie OR commune) NOT archive</em>
                      </p>
                    )}
                  </div>
                  <div>
              ...
              <label className="flex items-start gap-3 p-3 rounded-xl border-2 border-slate-200 dark:border-slate-600 cursor-pointer hover:border-slate-300 dark:hover:border-slate-500 transition-colors">
                ...
                />
                <div>
                  <p className="text-sm font-medium text-slate-900 dark:text-white">Analyse IA automatique</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Noter automatiquement (de 0 à 10) la pertinence de chaque correspondance de mots-clés à la fin du scraping, via OpenRouter. Nécessite une clé API configurée dans les Paramètres. Tu pourras aussi le lancer manuellement depuis l&apos;onglet Résultats.
                  </p>
                </div>
              </label>
            </>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-200 dark:border-slate-700">
          ...
          {step < 3 ? (
            ...
            >
              ...
              {isEdit ? 'Relancer' : 'Lancer le scraping'}
            </button>
          )}
        </div>
      </DialogContent>
